...
public class GLPainterHelper {
	...
	public static void plotDeformationResult(GL2 gl, Position position, StaticDeformationAlalysis analysis) throws Exception {
	   ...
	   // }
	    ...
	    for (IElement element : elements) {
	      ...
	      drawGlVertex3d_stress(gl, node0, analysis);
	      DrawGLColor3fStruct(gl,node1, analysis);
	      drawGlVertex3d_stress(gl,node1, analysis);
	      DrawGLColor3fStruct(gl,node2, analysis);
	      drawGlVertex3d_stress(gl,node2, analysis);
	      ...
	      drawGlVertex3d_stress(gl,node3, analysis);
	...
	// TODO Может быть переместить в лучшее место, может методы расширения
	  private static void drawGlVertex3d_stress(GL2 gl, INode node, StaticDeformationAlalysis analysis) throws Exception {
	    
	    int numberNode = analysis.getMesh().getNodes().lastIndexOf(node);
	    //Mulitple with cK
	    gl.glVertex3d(node.getValueOfDemention(Dim.X)+ analysis.getResult().getX()[numberNode], 
	      node.getValueOfDemention(Dim.Y)+analysis.getResult().getY()[numberNode],
	        node.getValueOfDemention(Dim.Z)+analysis.getResult().getZ()[numberNode]);
	  }
	...
	private static void DrawGLColor3fStruct(GL2 gl, INode node, StaticDeformationAlalysis analysis) {
		...
		setColorStructal(gl, analysis.getResult().getZ()[indexNode], analysis.getResult());
	}
	...
	private static int setColorStructal(GL2 gl, double value, StaticDeformationResult result) {
		    ...
		    double min = result.getZ()[0]; 
		    double max = result.getZ()[0];
		    
		    for(int i= 0;i<result.getZ().length;i++) {
		      if(min> result.getZ()[i]) {
		        min = result.getZ()[i];
		      }
		      if(max< result.getZ()[i]) {
		        max = result.getZ()[i];
		      }
		    }
